...
func Test_cos(t *testing.T) {
	op := cosOp[float64, *dense.Dense[float64]]{}
	// basic test
	...
	if expectedType, err = typecheck(op, a); err != nil {
		t.Fatalf("cosOp failed to typecheck. Err: %v", err)
	}

	if expectedShape, err = shapecheck(op, a); err != nil {
		t.Fatalf("cosOp failed to shapecheck. Err: %v", err)
	}

	if b, err = op.Do(context.Background(), a); err != nil {
		t.Fatalf("Expected cosOp{} to work correctly. Err: %v", err)
	}
