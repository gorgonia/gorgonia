...
func TestGt(t *testing.T) {
	...
	if expectedType, err = typecheck(op, a, b); err != nil {
		t.Fatalf("Expected Gt{} to pass type checking. Err: %v", err)
	}
	if expectedShape, err = shapecheck(op, a, b); err != nil {
		t.Fatalf("Expected Gt{} to pass shape checking. Err: %v", err)
	}

	// actually doing and testing
	if c, err = op.Do(context.Background(), a, b); err != nil {
		t.Fatalf("Expected Gt{} to work correctly. Err: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("Expected Gt{}'s Prealloc to work. Err: %v", err)
	}
	assert.Equal(t, expectedType, datatypes.TypeOf(c))
	assert.True(t, expectedShape.Eq(c.Shape()))
	correctScalar := false
	...
	if expectedType, err = typecheck(op, a, b); err == nil {
		t.Fatalf("Expected Gt{} to NOT pass type checking. Got ~(%v %v) =  %v ", datatypes.TypeOf(a), datatypes.TypeOf(b), expectedType)
	}
	if expectedShape, err = shapecheck(op, a, b); err == nil {
		t.Fatalf("Expected Gt{} to NOT pass shape checking. Got expectedShape = %v", expectedShape)
	}

}

func TestGtVS(t *testing.T) {
	op := GtVS{}
	// basic test
	assert.Equal(t, 2, op.Arity())
	...
	if expectedShape, err = shapecheck(op, a, b); err == nil {
		t.Fatalf("Expected Gt{} to NOT pass shape checking. Got %v ~ (%v, %v) = %v", op.ShapeExpr(), a.Shape(), b.Shape(), expectedShape)
	}
}
...
func TestGt_RetSame(t *testing.T) {
	op := Gt{retSame: true}
	// basic test
	assert.Equal(t, 2, op.Arity())
...
func TestGtVS_RetSame(t *testing.T) {
	op := GtVS{retSame: true}
	// basic test
	assert.Equal(t, 2, op.Arity())
...
func TestGtSV_RetSame(t *testing.T) {
	op := GtSV{retSame: true}
	// basic test
	assert.Equal(t, 2, op.Arity())
