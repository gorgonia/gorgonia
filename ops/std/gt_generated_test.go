...
func Test_gtVVRS_RetSame(t *testing.T) {
	op := gtVVRS[float64, *dense.Dense[float64]]{}
	// basic test
	...
	if expectedType, err = typecheck(op, a, b); err != nil {
		t.Fatalf("Expected gtVVRS{} to pass type checking. Err: %v", err)
	}
	if expectedShape, err = shapecheck(op, a, b); err != nil {
		t.Fatalf("Expected gtVVRS{} to pass shape checking. Err: %v", err)
	}

	// actually doing and testing
	if c, err = op.Do(context.Background(), a, b); err != nil {
		t.Fatalf("Expected gtVVRS{} to work correctly. Err: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("Expected gtVVRS{}'s Prealloc to work. Err: %v", err)
	}
	assert.Equal(t, expectedType, datatypes.TypeOf(c))
	assert.True(t, expectedShape.Eq(c.Shape()))
	correctScalar := 0.0
...
func Test_gtVSRS_RetSame(t *testing.T) {
	op := gtVSRS[float64, *dense.Dense[float64]]{}
	// basic test
	assert.Equal(t, 2, op.Arity())
	...
	// type and shape checks
	if expectedType, err = typecheck(op, a, b); err != nil {
		t.Fatalf("Expected gtVSRS{} to pass type checking. Err: %v", err)
	}
	if expectedShape, err = shapecheck(op, a, b); err != nil {
		t.Fatalf("Expected gtVSRS{} to pass shape checking. Err: %v", err)
	}

	// actually doing and test
	if c, err = op.Do(context.Background(), a, b); err != nil {
		t.Fatalf("Expected gtVSRS{} to work correctly. Err: %v", err)
	}
	assert.Equal(t, expectedType, datatypes.TypeOf(c))
	assert.True(t, expectedShape.Eq(c.Shape()))
	correct := []float64{0, 0, 0, 0, 0, 0}
	...
	c, err = op.PreallocDo(context.Background(), c, a, b)
	if err != nil {
		t.Fatalf("Expected gtVSRS{}'s Prealloc to work. Err: %v", err)
	}
	assert.Equal(t, expectedType, datatypes.TypeOf(c))
	assert.True(t, expectedShape.Eq(c.Shape()))
	assert.Equal(t, correct, c.Data())

	/* bad cases: gtVSRS{} on tensor-tensor */
...
func Test_gtSVRS_RetSame(t *testing.T) {
	op := gtSVRS[float64, *dense.Dense[float64]]{}
	// basic test
	assert.Equal(t, 2, op.Arity())
	...
	// type and shape checks
	if expectedType, err = typecheck(op, a, b); err != nil {
		t.Fatalf("Expected gtSVRS{} to pass type checking. Err: %v", err)
	}
	if expectedShape, err = shapecheck(op, a, b); err != nil {
		t.Fatalf("Expected gtSVRS{} to pass shape checking. Err: %v", err)
	}

	// actually doing and test
	if c, err = op.Do(context.Background(), a, b); err != nil {
		t.Fatalf("Expected gtSVRS{} to work correctly. Err: %v", err)
	}
	assert.Equal(t, expectedType, datatypes.TypeOf(c))
	assert.True(t, expectedShape.Eq(c.Shape()))
...
func Test_gtVV(t *testing.T) {
	op := gtVV[float64, *dense.Dense[float64], *dense.Dense[bool]]{}
	// basic test
	assert.Equal(t, 2, op.Arity())
...
func Test_gtVS(t *testing.T) {
	op := gtVS[float64, *dense.Dense[float64], *dense.Dense[bool]]{}
	// basic test
	assert.Equal(t, 2, op.Arity())
...
func Test_gtSV(t *testing.T) {
	op := gtSV[float64, *dense.Dense[float64], *dense.Dense[bool]]{}
	// basic test
	assert.Equal(t, 2, op.Arity())
