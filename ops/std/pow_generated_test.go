...
func TestPow(t *testing.T) {
	...
	if expectedType, err = typecheck(op, a, b); err == nil {
		t.Fatalf("Expected Pow{} to NOT pass type checking. Got ~(%v %v) =  %v ", datatypes.TypeOf(a), datatypes.TypeOf(b), expectedType)
	}
	if expectedShape, err = shapecheck(op, a, b); err == nil {
		t.Fatalf("Expected Pow{} to NOT pass shape checking. Got expectedShape = %v", expectedShape)
	}
...
func TestPowVS(t *testing.T) {
	...
	if expectedShape, err = shapecheck(op, a, b); err == nil {
		t.Fatalf("Expected Pow{} to NOT pass shape checking. Got %v ~ (%v, %v) = %v", op.ShapeExpr(), a.Shape(), b.Shape(), expectedShape)
	}
}
