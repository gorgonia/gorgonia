...
func Test_ln(t *testing.T) {
	op := lnOp[float64, *dense.Dense[float64]]{}
	// basic test
	...
	if expectedType, err = typecheck(op, a); err != nil {
		t.Fatalf("lnOp failed to typecheck. Err: %v", err)
	}

	if expectedShape, err = shapecheck(op, a); err != nil {
		t.Fatalf("lnOp failed to shapecheck. Err: %v", err)
	}

	if b, err = op.Do(context.Background(), a); err != nil {
		t.Fatalf("Expected lnOp{} to work correctly. Err: %v", err)
	}
	assert.Equal(t, expectedType, datatypes.TypeOf[float64](b))
	assert.True(t, expectedShape.Eq(b.Shape()))
	correct := []float64{math.Log(1), math.Log(2), math.Log(3), math.Log(4), math.Log(5), math.Log(6)}
