...
func Test_invSqrt(t *testing.T) {
	op := invSqrtOp[float64, *dense.Dense[float64]]{}
	// basic test
	...
	if expectedType, err = typecheck(op, a); err != nil {
		t.Fatalf("invSqrtOp failed to typecheck. Err: %v", err)
	}

	if expectedShape, err = shapecheck(op, a); err != nil {
		t.Fatalf("invSqrtOp failed to shapecheck. Err: %v", err)
	}

	if b, err = op.Do(context.Background(), a); err != nil {
		t.Fatalf("Expected invSqrtOp{} to work correctly. Err: %v", err)
	}
	assert.Equal(t, expectedType, datatypes.TypeOf(b))
	assert.True(t, expectedShape.Eq(b.Shape()))
	correct := []float64{1 / math.Sqrt(1), 1 / math.Sqrt(2), 1 / math.Sqrt(3), 1 / math.Sqrt(4), 1 / math.Sqrt(5), 1 / math.Sqrt(6)}
