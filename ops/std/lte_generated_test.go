...
func TestLte(t *testing.T) {
	...
	if expectedType, err = typecheck(op, a, b); err != nil {
		t.Fatalf("Expected Lte{} to pass type checking. Err: %v", err)
	}
	if expectedShape, err = shapecheck(op, a, b); err != nil {
		t.Fatalf("Expected Lte{} to pass shape checking. Err: %v", err)
	}

	// actually doing and testing
	if c, err = op.Do(context.Background(), a, b); err != nil {
		t.Fatalf("Expected Lte{} to work correctly. Err: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("Expected Lte{}'s Prealloc to work. Err: %v", err)
	}
	assert.Equal(t, expectedType, datatypes.TypeOf(c))
	assert.True(t, expectedShape.Eq(c.Shape()))
	correctScalar := true
	...
	if expectedType, err = typecheck(op, a, b); err == nil {
		t.Fatalf("Expected Lte{} to NOT pass type checking. Got ~(%v %v) =  %v ", datatypes.TypeOf(a), datatypes.TypeOf(b), expectedType)
	}
	if expectedShape, err = shapecheck(op, a, b); err == nil {
		t.Fatalf("Expected Lte{} to NOT pass shape checking. Got expectedShape = %v", expectedShape)
	}
...
func TestLteVS(t *testing.T) {
	...
	if expectedShape, err = shapecheck(op, a, b); err == nil {
		t.Fatalf("Expected Lte{} to NOT pass shape checking. Got %v ~ (%v, %v) = %v", op.ShapeExpr(), a.Shape(), b.Shape(), expectedShape)
	}
}
...
func TestLte_RetSame(t *testing.T) {
	op := Lte{retSame: true}
	// basic test
	assert.Equal(t, 2, op.Arity())
