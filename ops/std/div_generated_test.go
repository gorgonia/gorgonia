...
func TestDiv(t *testing.T) {
	...
	if expectedType, err = typecheck(op, a, b); err != nil {
		t.Fatalf("Expected Div{} to pass type checking. Err: %v", err)
	}
	if expectedShape, err = shapecheck(op, a, b); err != nil {
		t.Fatalf("Expected Div{} to pass shape checking. Err: %v", err)
	}

	if c, err = op.Do(context.Background(), a, b); err != nil {
		t.Fatalf("Expected Div{} to work correctly. Err: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("Expected Div{}'s Prealloc to work. Err: %v", err)
	}
	correctScalar := 0.5
	assert.Equal(t, expectedType, datatypes.TypeOf(c))
	...
	if expectedType, err = typecheck(op, a, b); err == nil {
		t.Fatalf("Expected Div{} to NOT pass type checking. Got ~(%v %v) =  %v ", datatypes.TypeOf(a), datatypes.TypeOf(b), expectedType)
	}
	if expectedShape, err = shapecheck(op, a, b); err == nil {
		t.Fatalf("Expected Div{} to NOT pass shape checking. Got expectedShape = %v", expectedShape)
	}
...
func TestDivVS(t *testing.T) {
	...
	if expectedShape, err = shapecheck(op, a, b); err == nil {
		t.Fatalf("Expected Div{} to NOT pass shape checking. Got %v ~ (%v, %v) = %v", op.ShapeExpr(), a.Shape(), b.Shape(), expectedShape)
	}
}

func TestDivSV(t *testing.T) {
	op := DivSV{}
