...
func (op Gt) Do(ctx context.Context, vs ...values.Value) (retVal values.Value, err error) {
	...
	if op.retSame {
		retVal, err = tensor.Gt(a, b, tensor.WithContext(ctx2), tensor.AsSame())
	} else {
...
func (op Gt) PreallocDo(ctx context.Context, prealloc values.Value, vs ...values.Value) (retVal values.Value, err error) {
	...
	ctx2, task := trace.NewTask(ctx, op.String())
	if op.retSame {
		retVal, err = tensor.Gt(a, b, tensor.WithReuse(prealloc), tensor.WithContext(ctx2), tensor.AsSame())
	} else {
...
func (op GtSV) PreallocDo(ctx context.Context, prealloc values.Value, vs ...values.Value) (values.Value, error) {
	if err := handleCtx(ctx); err != nil {
		return nil, err
	}
