...
func Test_neg(t *testing.T) {
	op := negOp[float64, *dense.Dense[float64]]{}
	// basic test
	...
	if expectedType, err = typecheck(op, a); err != nil {
		t.Fatalf("negOp failed to typecheck. Err: %v", err)
	}

	if expectedShape, err = shapecheck(op, a); err != nil {
		t.Fatalf("negOp failed to shapecheck. Err: %v", err)
	}

	if b, err = op.Do(context.Background(), a); err != nil {
		t.Fatalf("Expected negOp{} to work correctly. Err: %v", err)
	}
