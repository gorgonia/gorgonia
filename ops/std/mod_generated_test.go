...
func TestMod(t *testing.T) {
	...
	if expectedType, err = typecheck(op, a, b); err != nil {
		t.Fatalf("Expected Mod{} to pass type checking. Err: %v", err)
	}
	if expectedShape, err = shapecheck(op, a, b); err != nil {
		t.Fatalf("Expected Mod{} to pass shape checking. Err: %v", err)
	}

	if c, err = op.Do(context.Background(), a, b); err != nil {
		t.Fatalf("Expected Mod{} to work correctly. Err: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("Expected Mod{}'s Prealloc to work. Err: %v", err)
	}
	correctScalar := 1.0
	assert.Equal(t, expectedType, datatypes.TypeOf(c))
	...
	if expectedType, err = typecheck(op, a, b); err == nil {
		t.Fatalf("Expected Mod{} to NOT pass type checking. Got ~(%v %v) =  %v ", datatypes.TypeOf(a), datatypes.TypeOf(b), expectedType)
	}
	if expectedShape, err = shapecheck(op, a, b); err == nil {
		t.Fatalf("Expected Mod{} to NOT pass shape checking. Got expectedShape = %v", expectedShape)
	}

}

func TestModVS(t *testing.T) {
	op := ModVS{}
	...
	if expectedShape, err = shapecheck(op, a, b); err == nil {
		t.Fatalf("Expected Mod{} to NOT pass shape checking. Got %v ~ (%v, %v) = %v", op.ShapeExpr(), a.Shape(), b.Shape(), expectedShape)
	}
}

func TestModSV(t *testing.T) {
	op := ModSV{}
