...
func Test_exp(t *testing.T) {
	op := expOp[float64, *dense.Dense[float64]]{}
	// basic test
	...
	if expectedType, err = typecheck(op, a); err != nil {
		t.Fatalf("expOp failed to typecheck. Err: %v", err)
	}

	if expectedShape, err = shapecheck(op, a); err != nil {
		t.Fatalf("expOp failed to shapecheck. Err: %v", err)
	}

	if b, err = op.Do(context.Background(), a); err != nil {
		t.Fatalf("Expected expOp{} to work correctly. Err: %v", err)
	}
	assert.Equal(t, expectedType, datatypes.TypeOf[float64](b))
	assert.True(t, expectedShape.Eq(b.Shape()))
	correct := []float64{math.Exp(1), math.Exp(2), math.Exp(3), math.Exp(4), math.Exp(5), math.Exp(6)}
