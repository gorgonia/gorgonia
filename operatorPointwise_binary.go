...
func (o scalarBinOp) Do(same bool, vals ...value.Value) (retVal value.Value, err error) {
	...
	case *primitive.F64:
		b := vals[1].(*primitive.F64)
		switch o.ʘBinaryOperatorType {
		case addOpType:
			r = primitive.NewF64(a.Any() + b.Any())
		case subOpType:
			r = primitive.NewF64(a.Any() - b.Any())
		case mulOpType:
			r = primitive.NewF64(a.Any() * b.Any())
		case divOpType:
			r = primitive.NewF64(a.Any() / b.Any())
		case powOpType:
			r = primitive.NewF64(math.Pow(a.Any(), b.Any()))
		case ltOpType:
			r = primitive.NewB(a.Any() < b.Any())
		case gtOpType:
			r = primitive.NewB(a.Any() > b.Any())
		case lteOpType:
			r = primitive.NewB(a.Any() <= b.Any())
		...
		if same && !o.isArith() {
			if *(r.(*primitive.B)) {
				r = primitive.NewF64(1.0)
			} else {
				r = primitive.NewF64(0.0)
			}
		}

	case *primitive.F32:
		b := vals[1].(*primitive.F32)
		switch o.ʘBinaryOperatorType {
		case addOpType:
			r = primitive.NewF32(a.Any() + b.Any())
		case subOpType:
			r = primitive.NewF32(a.Any() - b.Any())
		case mulOpType:
			r = primitive.NewF32(a.Any() * b.Any())
		case divOpType:
			r = primitive.NewF32(a.Any() / b.Any())
		case powOpType:
			r = primitive.NewF32(math32.Pow(float32(a.Any()), float32(b.Any())))
		case ltOpType:
			r = primitive.NewB(a.Any() < b.Any())
		case gtOpType:
			r = primitive.NewB(a.Any() > b.Any())
	...
	case *primitive.I:
		b := vals[1].(*primitive.I)
		switch o.ʘBinaryOperatorType {
		case addOpType:
			r = primitive.NewI(a.Any() + b.Any())
		case subOpType:
			r = primitive.NewI(a.Any() - b.Any())
		case mulOpType:
			r = primitive.NewI(a.Any() * b.Any())
	...
	case *primitive.I32:
		b := vals[1].(*primitive.I32)
		switch o.ʘBinaryOperatorType {
		case addOpType:
			r = primitive.NewI32(a.Any() + b.Any())
		case subOpType:
			r = primitive.NewI32(a.Any() - b.Any())
		case mulOpType:
			r = primitive.NewI32(a.Any() * b.Any())
	...
	case *primitive.I64:
		b := vals[1].(*primitive.I64)
		switch o.ʘBinaryOperatorType {
		case addOpType:
			r = primitive.NewI64(a.Any() + b.Any())
		case subOpType:
			r = primitive.NewI64(a.Any() - b.Any())
		case mulOpType:
			r = primitive.NewI64(a.Any() * b.Any())
	...
	case *primitive.U8:
		b := vals[1].(*primitive.U8)
		switch o.ʘBinaryOperatorType {
		case addOpType:
			r = primitive.NewU8(a.Any() + b.Any())
		case subOpType:
			r = primitive.NewU8(a.Any() - b.Any())
		case mulOpType:
			r = primitive.NewU8(a.Any() * b.Any())
	...
	retVal, _ = primitive.AnyToScalar(r)
	return
}
...
func hadamardPowDiff(ctx execution.Context, x, y, z *Node) (err error) {
	...
	case *primitive.F64:
		ym1 = primitive.NewF64(ydvt.Any() - float64(1))
	case *primitive.F32:
		ym1 = primitive.NewF32(ydvt.Any() - float32(1))
	case *tensor.Dense:
		var one interface{}
		switch ydvt.Dtype() {
		case tensor.Float64:
