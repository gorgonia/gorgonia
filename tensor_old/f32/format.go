...
func (t *Tensor) Format(state fmt.State, c rune) {
	...
	for _, v := range consideration {
		buf = strconv.AppendFloat(buf, float64(v), format, precision, 64)
		if len(buf) > width {
			width = len(buf)
		}
		// empty buffer
	...
	if flat {
		...
		case extended:
			for i, v := range t.data {
				buf = strconv.AppendFloat(buf[:0], float64(v), format, precision, 64)
				state.Write(buf)
				if i < len(t.data)-1 {
					state.Write(pad[:1])
				}
		...
		case t.viewOf != nil:
			...
			for i, err = it.Next(); err == nil; i, err = it.Next() {
				...
				buf = strconv.AppendFloat(buf[:0], float64(t.data[i]), format, precision, 64)
				state.Write(buf)
				state.Write(pad[:1])
