...
func scalarCmpBacking(op cmpOp, left bool, val float32, data []float32) (backing []bool, err error) {
...
func (t *Tensor) scalarCmp(op cmpOp, left bool, val float32) (retVal *tb.Tensor, err error) {
	var backing []bool
...
func (t *Tensor) tensorCmp(op cmpOp, other *Tensor, boolT bool) (retVal types.Tensor, err error) {
	...
	if !boolT {
		backingF := boolsToFloat32s(backing)
