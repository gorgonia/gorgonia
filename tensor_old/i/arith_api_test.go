...
func TestAdd(t *testing.T) {
	...
	t.Logf("T-T")
	backingA := []int{1, 2, 3, 4, 5}
	backingB := []int{5, 4, 3, 2, 1}
	backingR := []int{1, 3, 5, 6, 9}
	correct = []int{6, 6, 6, 6, 6}
	...
	t.Logf("T-s")
	backingA = []int{1, 2, 3, 4, 5}
	correct = []int{6, 7, 8, 9, 10}
	backingR = []int{1, 2, 3, 4, 5}
	Ta = NewTensor(WithBacking(backingA))
	reuse = NewTensor(WithBacking(backingR))

	// safe (default use case)
	if got, err = Add(Ta, int(5)); err != nil {
		t.Error(err)
	}
	if got == Ta {
		t.Error(safeOpErr)
	...
	// with incr
	if got, err = Add(Ta, int(5), types.WithIncr(reuse)); err != nil {
		t.Error(err)
	}
	if expected = reuse; got != expected {
		t.Error(reuseOpErr)
	}
	correct = []int{7, 9, 11, 13, 15}
	assert.Equal(correct, got.data)

	// with reuse
	if got, err = Add(Ta, int(5), types.WithReuse(reuse)); err != nil {
		t.Error(err)
	}
	...
	// unsafe
	if got, err = Add(Ta, int(5), types.UseUnsafe()); err != nil {
		t.Error(err)
	}
	...
	t.Logf("s-T")
	correct = []int{10, 9, 8, 7, 6}
	backingR = []int{1, 2, 3, 4, 5}
	reuse = NewTensor(WithBacking(backingR))

	// safe (default use case)
	if got, err = Add(int(5), Tb); err != nil {
		t.Error(err)
	}
	if got == Tb {
		t.Error(safeOpErr)
	...
	// with incr
	if got, err = Add(int(5), Tb, types.WithIncr(reuse)); err != nil {
		t.Error(err)
	}
	if expected = reuse; got != expected {
		t.Error(reuseOpErr)
	}
	correct = []int{11, 11, 11, 11, 11}
	assert.Equal(correct, got.data)

	// with reuse
	if got, err = Add(int(5), Tb, types.WithReuse(reuse)); err != nil {
		t.Error(err)
	}
	...
	// unsafe
	if got, err = Add(int(5), Tb, types.UseUnsafe()); err != nil {
		t.Error(err)
	}
	...
	t.Logf("s-s")
	if got, err = Add(int(5), int(5)); err == nil {
...
func TestSub(t *testing.T) {
	...
	backingA = []int{1, 2, 3, 4, 5}
	backingR = []int{1, 3, 5, 6, 9}
	correct = []int{-4, -3, -2, -1, 0}
	reuse = NewTensor(WithBacking(backingR))
	Ta = NewTensor(WithBacking(backingA))
	if got, err = Sub(Ta, int(5)); err != nil {
		t.Error(err)
	}
	if got == Ta {
		t.Error(safeOpErr)
	...
	// with incr
	if got, err = Sub(Ta, int(5), types.WithIncr(reuse)); err != nil {
		t.Error(err)
	}
	...
	correct = []int{-3, 0, 3, 5, 9}
	assert.Equal(correct, got.data)

	// with reuse
	if got, err = Sub(Ta, int(5), types.WithReuse(reuse)); err != nil {
		t.Error(err)
	}
	...
	// unsafe
	if got, err = Sub(Ta, int(5), types.UseUnsafe()); err != nil {
		t.Error(err)
	}
	if got != Ta {
		t.Error(unsafeOpErr)
	...
	if got, err = Sub(int(5), Tb); err != nil {
		t.Error(err)
	}
	if got == Tb {
		t.Error(safeOpErr)
	...
	backingR = []int{1, 3, 5, 6, 9}
	reuse = NewTensor(WithBacking(backingR))
	if got, err = Sub(int(5), Tb, types.WithIncr(reuse)); err != nil {
		t.Error(err)
	}
	...
	correct = []int{1, 4, 7, 9, 13}
	assert.Equal(correct, got.data)

	// with reuse
	if got, err = Sub(int(5), Tb, types.WithReuse(reuse)); err != nil {
		t.Error(err)
	}
	...
	// unsafe
	if got, err = Sub(int(5), Tb, types.UseUnsafe()); err != nil {
		t.Error(err)
	}
	if got != Tb {
		t.Error(unsafeOpErr)
	...
	/* Idiots */
	t.Logf("s-s")
	if got, err = Sub(int(5), int(5)); err == nil {
		t.Error("Expected an error, duh doy")
	}
...
func TestPointwiseMul(t *testing.T) {
	...
	backingA := []int{1, 2, 3, 4, 5}
	backingB := []int{5, 4, 3, 2, 1}
	backingR := []int{1, 3, -1, -5, 2}
	correct = []int{5, 8, 9, 8, 5}
	Ta := NewTensor(WithBacking(backingA))
	Tb := NewTensor(WithBacking(backingB))
	reuse := NewTensor(WithBacking(backingR))
	...
	if got, err = PointwiseMul(Ta, int(5)); err != nil {
		t.Error(err)
	}
	if got == Ta || got == Tb {
		t.Error(safeOpErr)
	...
	correct = []int{5, 10, 15, 20, 25}
	assert.Equal(correct, got.data)

	// with incr
	backingR = []int{1, 3, -11, 2, 6}
	reuse = NewTensor(WithBacking(backingR))
	if got, err = PointwiseMul(Ta, int(5), types.WithIncr(reuse)); err != nil {
		t.Error(err)
	}
	...
	correct = []int{6, 13, 4, 22, 31}
	assert.Equal(correct, got.data)

	// with reuse
	if got, err = PointwiseMul(Ta, int(5), types.WithReuse(reuse)); err != nil {
		t.Error(err)
	}
	...
	// unsafe
	if got, err = PointwiseMul(Ta, int(5), types.UseUnsafe()); err != nil {
		t.Error(err)
	}
	if got != Ta {
		t.Error(unsafeOpErr)
	...
	t.Logf("s-T")
	if got, err = PointwiseMul(int(5), Tb); err != nil {
		t.Error(err)
	}
	if got == Ta || got == Tb {
		t.Error(safeOpErr)
	}
	correct = []int{25, 20, 15, 10, 5}
	assert.Equal(correct, got.data)
	...
	backingR = []int{1, 3, -11, 2, 6}
	reuse = NewTensor(WithBacking(backingR))
	if got, err = PointwiseMul(int(5), Tb, types.WithIncr(reuse)); err != nil {
		t.Error(err)
	}
	...
	correct = []int{26, 23, 4, 12, 11}
	assert.Equal(correct, got.data)

	// with reuse
	if got, err = PointwiseMul(int(5), Tb, types.WithReuse(reuse)); err != nil {
		t.Error(err)
	}
	...
	// unsafe
	if got, err = PointwiseMul(int(5), Tb, types.UseUnsafe()); err != nil {
		t.Error(err)
	}
	if got != Tb {
		t.Error(unsafeOpErr)
	...
	/* Idiots */
	t.Logf("s-s")
	if got, err = PointwiseMul(int(5), int(5)); err == nil {
		t.Error("Expected an error, duh doy")
	}
...
func TestPointwiseDiv(t *testing.T) {
	...
	correct = []int{int(1) / int(5), int(2) / int(4), 1, 2, 5}
	assert.Equal(correct, got.data)
	...
	if got, err = PointwiseDiv(Ta, int(5)); err != nil {
		t.Error(err)
	}
	expected = Ta
	if got == expected {
	...
	correct = []int{int(1) / int(5), int(2) / int(5), int(3) / int(5), int(4) / int(5), 1}
	// assert.Equal(correct, got.data)
	...
	t.Logf("s-T")
	if got, err = PointwiseDiv(int(5), Tb); err != nil {
		t.Error(err)
	}
	expected = Tb
	if got == expected {
	...
	correct = []int{1, int(5) / int(4), int(5) / int(3), int(5) / int(2), 5}
	assert.Equal(correct, got.data)

	/* Idiots */
	t.Logf("s-s")
	if got, err = PointwiseDiv(int(5), int(5)); err == nil {
		t.Error("Expected an error, duh doy")
	}
...
func TestPointwisePow(t *testing.T) {
	...
	if got, err = PointwisePow(Ta, int(2)); err != nil {
		t.Error(err)
	}
	expected = Ta
	if got == expected {
	...
	t.Logf("s-T")
	if got, err = PointwisePow(int(2), Tb); err != nil {
		t.Error(err)
	}
	expected = Tb
	if got == expected {
	...
	correct = []int{1, 2, 4, 8, 16}
	assert.Equal(correct, got.data)

	/* Idiots */
	t.Logf("s-s")
	if got, err = PointwisePow(int(5), int(5)); err == nil {
		t.Error("Expected an error, duh doy")
	}
