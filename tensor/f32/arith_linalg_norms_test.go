...
func testNormVal(T *Tensor, ord types.NormOrder, want float32) error {
	...
	got := retVal.ScalarValue().(float32)
	if !isclose(want, got) && !(math32.IsNaN(want) && alike(want, got)) {
...
func TestTensor_Norm(t *testing.T) {
	var T *Tensor
	var err error
	var backing, backing1, backing2 []float32
	var corrects map[types.NormOrder]float32
	...
	backing1 = []float32{-1, -2, -3, -4}
	backing2 = []float32{-1, 2, -3, 4}

	corrects = map[types.NormOrder]float32{
		types.UnorderedNorm(): math32.Pow(30, 0.5),               // Unordered
		types.FrobeniusNorm(): math32.NaN(),                      // Frobenius
		types.NuclearNorm():   math32.NaN(),                      // Nuclear
		types.InfNorm():       4,                                 // Inf
		types.NegInfNorm():    1,                                 // -Inf
		types.Norm(0):         4,                                 // 0
		types.Norm(1):         10,                                // 1
		types.Norm(-1):        12.0 / 25.0,                       // -1
		types.Norm(2):         math32.Pow(30, 0.5),               // 2
		types.Norm(-2):        math32.Pow((205.0 / 144.0), -0.5), // -2
	}
	T = NewTensor(WithShape(len(backing)))

	backings := [][]float32{backing, backing1, backing2}
	...
	// 2x2 mat
	backing = []float32{1, 3, 5, 7}
	corrects = map[types.NormOrder]float32{
		types.UnorderedNorm(): math32.Pow(84, 0.5), // Unordered
		types.FrobeniusNorm(): math32.Pow(84, 0.5), // Frobenius
	...
	for _, ord := range wrongs {
		if err = testNormVal(T, ord, math32.NaN()); err == nil {
	...
	backing = []float32{0.1, 0.2, 0.3, 0.6, 0, 0.5, 0.3, 0.2, 0.1}
	corrects = map[types.NormOrder]float32{
		types.FrobeniusNorm(): (1.0 / 10.0) * math32.Pow(89, 0.5),
