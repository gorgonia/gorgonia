...
func TestVecVecDot(t *testing.T) {
	...
	// b is not a vector
	a = NewTensor(WithShape(3, 1), WithBacking(RangeFloat32(0, 3)))
	b = NewTensor(WithShape(3, 2), WithBacking(RangeFloat32(0, 6)))
	_, err = a.Inner(b)
	assert.NotNil(err)

	// shape mismatch
	a = NewTensor(WithShape(3, 1), WithBacking(RangeFloat32(0, 3)))
	b = NewTensor(WithShape(4, 1), WithBacking(RangeFloat32(0, 4)))
	r, err = a.Inner(b)
	assert.NotNil(err)
	...
	a = NewTensor(WithShape(3, 1), WithBacking(RangeFloat32(0, 3)))
	b = NewTensor(WithShape(1, 3), WithBacking(RangeFloat32(0, 3)))
	r, err = a.Inner(b)
...
func TestTMatVecMul(t *testing.T) {
	...
	A = NewTensor(WithShape(2, 3), WithBacking(RangeFloat32(0, 6)))
	b = NewTensor(WithShape(3, 1), WithBacking(RangeFloat32(0, 3)))
	...
	incr = NewTensor(WithShape(2, 1), WithBacking([]float32{50, 100}))
	...
	// stupid inputs - 1
	A = NewTensor(WithShape(2, 1, 3), WithBacking(RangeFloat32(0, 6)))
	b = NewTensor(WithShape(3, 1), WithBacking(RangeFloat32(0, 3)))
	r, err = A.MatVecMul(b)
	assert.NotNil(err)
	...
	// stupid inputs - 2
	A = NewTensor(WithShape(2, 3), WithBacking(RangeFloat32(0, 6)))
	b = NewTensor(WithShape(3, 3), WithBacking(RangeFloat32(0, 9)))
	_, err = A.MatVecMul(b)
	assert.NotNil(err)
	...
	A.data = []float32{1, -1, 2, 0, -3, 1}
	b.data = []float32{2, 1, 0}
	r, err = A.MatVecMul(b)
	assert.Equal([]float32{1, -3}, r.data)
}
...
func TestTMatMul(t *testing.T) {
	...
	expectedData = []float32{10, 13, 28, 40}
	...
	// With Incr provided
	incr = NewTensor(WithShape(2, 2), WithBacking([]float32{100, 200, -200, -100}))
	...
	expectedData = []float32{110, 213, -172, -60}
	expectedShape = types.Shape{2, 2}
	...
	// B is not 2D matrix
	A = NewTensor(WithShape(2, 3), WithBacking(RangeFloat32(0, 6)))
	B = NewTensor(WithShape(3, 1, 2), WithBacking(RangeFloat32(0, 6)))
	_, err = A.MatMul(B)
	assert.NotNil(err)
...
func TestTmatMul(t *testing.T) {
	...
	A.T()
	A.matMul(B, R)
	expectedData = []float32{20, 26, 26, 35}
	expectedShape = types.Shape{2, 2}
	...
	B.T()
	A.matMul(B, R)
	expectedData = []float32{5, 14, 14, 50}
	expectedShape = types.Shape{2, 2}
	...
	B.T()
	A.matMul(B, R)
	expectedData = []float32{3, 9, 15, 4, 14, 24, 5, 19, 33}
...
func TestTOuter(t *testing.T) {
	...
	b = NewTensor(WithShape(3, 1), WithBacking(RangeFloat32(0, 3)))
	R, err = a.Outer(b)
	expectedData = []float32{0, 0, 0, 0, 1, 2, 0, 2, 4}
	expectedShape = types.Shape{3, 3}
	assert.Nil(err)
	assert.Equal(expectedData, R.data)
	assert.Equal(expectedShape, R.Shape())
	...
	a = NewTensor(WithShape(3, 1), WithBacking(RangeFloat32(1, 4)))
	b = NewTensor(WithShape(3, 1), WithBacking(RangeFloat32(0, 3)))
	R = NewTensor(WithShape(3, 3))
	...
	expectedData = []float32{0, 1, 2, 0, 2, 4, 0, 3, 6}
	expectedShape = types.Shape{3, 3}
	assert.Nil(err, err)
	assert.Equal(expectedData, R.data)
	assert.Equal(expectedShape, R.Shape())
	...
	// with incr
	incr = NewTensor(WithShape(3, 3), WithBacking([]float32{100, 200, 300, 400, 500, 600, 700, 800, 900}))
	R, err = a.Outer(b, types.WithIncr(incr))
	if R != incr {
		t.Error("incr not returned")
	}

	expectedData = []float32{100, 200, 300, 400, 501, 602, 700, 802, 904}
	expectedShape = types.Shape{3, 3}
	assert.Nil(err, err)
	assert.Equal(expectedData, R.data)
	assert.Equal(expectedShape, R.Shape())
...
func TestTouter(t *testing.T) {
	...
	R = NewTensor(WithShape(3, 4))
	a.outer(b, R)
	expectedData = []float32{0, 0, 0, 0, 0, 1, 2, 3, 0, 2, 4, 6}
...
func TestTensorMul(t *testing.T) {
	...
	A = NewTensor(WithShape(3, 4, 5), WithBacking(RangeFloat32(0, 60)))
	B = NewTensor(WithShape(4, 3, 2), WithBacking(RangeFloat32(0, 24)))
