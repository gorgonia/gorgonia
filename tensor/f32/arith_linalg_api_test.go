...
func TestDot(t *testing.T) {
	...
	// mat-mat
	t.Log("Mat⋅Mat")
	A = NewTensor(WithShape(4, 5), WithBacking(RangeFloat32(0, 20)))
	B = NewTensor(WithShape(5, 10), WithBacking(RangeFloat32(2, 52)))
	R, err = Dot(A, B)
	...
	// T-T
	t.Log("3T⋅3T")
	A = NewTensor(WithShape(2, 3, 4), WithBacking(RangeFloat32(0, 24)))
	B = NewTensor(WithShape(3, 4, 2), WithBacking(RangeFloat32(0, 24)))
	R, err = Dot(A, B)
	...
	t.Log("3T⋅4T")
	A = NewTensor(WithShape(2, 3, 4), WithBacking(RangeFloat32(0, 24)))
	B = NewTensor(WithShape(2, 3, 4, 5), WithBacking(RangeFloat32(0, 120)))
	R, err = Dot(A, B)
	...
	// T-v
	t.Log("3T⋅Vec")
	b = NewTensor(WithShape(4), WithBacking(RangeFloat32(0, 4)))
	R, err = Dot(A, b)
	...
	// m-3T
	t.Log("Mat⋅3T")
	A = NewTensor(WithShape(2, 4), WithBacking(RangeFloat32(0, 8)))
	B = NewTensor(WithShape(2, 4, 5), WithBacking(RangeFloat32(0, 40)))
	R, err = Dot(A, B)
	...
	R = NewTensor(WithShape(2))
	a = NewTensor(WithShape(4), WithBacking(RangeFloat32(0, 4)))
	B = NewTensor(WithShape(4, 2), WithBacking(RangeFloat32(0, 8)))
	R2, err = Dot(a, B, types.WithReuse(R))
	expectedShape = types.Shape{2}
	expectedData = []float32{28, 34}
	assert.Nil(err)
	assert.Equal(R, R2)
	assert.Equal(expectedData, R.data)
	assert.Equal(expectedShape, R.Shape())
	...
	assert.Equal(float32(50), R.data[0])

	R.Zero()
	R2, err = Dot(s, s2, types.WithReuse(R))
	assert.Nil(err)
	assert.True(R2.IsScalar())
	assert.Equal(float32(50), R2.data[0])

	R, err = Dot(s, A)
	expectedData = RangeFloat32(0, 24)
	vecScale(float32(5), expectedData)
	assert.Nil(err)
	...
	incr = NewTensor(AsScalar(float32(50)))
	R, err = Dot(s, s2, types.WithIncr(incr))
	assert.Nil(err)
	assert.True(R.IsScalar())
	assert.Equal(float32(100), R.data[0])
