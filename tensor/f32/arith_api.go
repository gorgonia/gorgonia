...
// Add performs a pointwise a+b. a and b can either be float32 or *Tensor
//
...
// Sub performs a pointwise a-b . a and b can either be float32 or *Tensor
//
// If both operands are *Tensor, shape is checked first.
// Even though the underlying data may have the same size (say (2,2) vs (4,1)), if they have different shapes, it will error out.
//
...
// PointwiseMul performs a pointwise a * b. a and b can either be float32 or *Tensor
//
// If both operands are *Tensor, shape is checked first.
// Even though the underlying data may have the same size (say (2,2) vs (4,1)), if they have different shapes, it will error out.
//
...
// PointwiseDiv performs a pointwise a / b. Valid values are either float32 or *Tensor.
//
// If both operands are *Tensor, shape is checked first.
// Even though the underlying data may have the same size (say (2,2) vs (4,1)), if they have different shapes, it will error out.
//
...
func PointwiseDiv(a, b interface{}, opts ...types.FuncOpt) (retVal *Tensor, err error) {
	...
	case incr && atok && btok:
		...
		retVal = reuse
	case incr && atok && bfok:
		incrVecScale(reuse.data, at.data, float32(1)/bf)
		retVal = reuse
	case incr && afok && btok:
	...
	case toReuse && atok && btok:
		...
		retVal = reuse
	case toReuse && atok && bfok:
		safeVecScale(float32(1)/bf, at.data, reuse.data)
		retVal = reuse
	case toReuse && afok && btok:
	...
	case safe && atok && bfok:
		retVal = newBorrowedTensor(len(at.data))
		retVal.setShape(at.Shape()...)
		safeVecScale(float32(1)/bf, at.data, retVal.data)
	case safe && afok && btok:
		retVal = newBorrowedTensor(len(bt.data))
		retVal.setShape(bt.Shape()...)
...
// PointwisePow performs a pointwise a ^ b. Valid values are either float32 or *Tensor.
//
// If both operands are *Tensor, shape is checked first.
// Even though the underlying data may have the same size (say (2,2) vs (4,1)), if they have different shapes, it will error out.
//
