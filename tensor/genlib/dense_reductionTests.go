...
const testDenseReduceRaw = `var denseReductionTests = []struct {
	...
	{{if isNumber . -}}
	// {{.}}
	{ {{asType . | title}}, add{{short .}}, {{asType .}}(0), 0, []{{asType .}}{6, 8, 10, 12, 14, 16}, Shape{3,2} },
	{ {{asType . | title}}, add{{short .}}, {{asType .}}(0), 1, []{{asType .}}{6, 9, 24, 27}, Shape{2, 2}},
	{ {{asType . | title}}, add{{short .}}, {{asType .}}(0), 2, []{{asType .}}{1, 5, 9, 13, 17, 21}, Shape{2, 3}},
	{{end -}}
	{{end -}}
}
...
func denseReductionTests(f io.Writer, generic *ManyKinds) {
