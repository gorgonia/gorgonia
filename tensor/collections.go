...
func denseTensorsToTensors(a []DenseTensor) []Tensor {
	retVal := make([]Tensor, len(a))
	for i, t := range a {
		retVal[i] = t
	}
...
func denseTensorsToDenses(a []DenseTensor) ([]*Dense, error) {
	retVal := make([]*Dense, len(a))
	var ok bool
	for i, t := range a {
		if retVal[i], ok = t.(*Dense); !ok {
			return nil, errors.Errorf("can only convert DenseTensors of *Dense. Trying to convert %T (#%d in slice)", t, i)
		}
	}
	return retVal, nil
}
