...
type NodeSet map[*node.Node]struct{}

// NewNodeSet creates and returns a reference to an empty set.
func NewNodeSet(a ...*node.Node) NodeSet {
	s := make(NodeSet)
	for _, i := range a {
		s.Add(i)
	}
...
func (set NodeSet) Add(i *node.Node) bool {
	_, found := set[i]
	set[i] = struct{}{}
...
func (set NodeSet) Contains(i *node.Node) bool {
	_, found := set[i]
	return found
}

// ContainsAll determines if the given items are all in the set
func (set NodeSet) ContainsAll(i ...*node.Node) bool {
	for _, v := range i {
		if !set.Contains(v) {
			return false
		}
...
func (set NodeSet) Remove(i *node.Node) {
	delete(set, i)
}
...
// Iter returns a channel of type *node.Node that you can range over.
func (set NodeSet) Iter() <-chan *node.Node {
