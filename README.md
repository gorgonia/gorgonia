...
The main reason for the major split in repositories is that `tensor` has found a life outside being used by Gorgonia proper. Quite a bit of attention and care has been given to the subpackage and as a result it's fairly performant on its own. 

There will be other repositories that will be moved to the Gorgonia organization as well (like `cu` and `vecf64`). Those will be enumerated in the README of their own repositories
