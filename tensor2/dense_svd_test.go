...
func testSVD(T, T2, s, u, v *Dense, t string, i int) (err error) {
	...
	if !sliceApproxf64(T2.Data().([]float64), T.Data().([]float64), closeenoughf64) {
	...
	if !sliceApproxf64(T2.Data().([]float64), reconstructed.Data().([]float64), closeenoughf64) {
...
func TestT_SVD(t *testing.T) {
	...
	for i, stts := range svdtestsThin {
		...
		if !sliceApproxf64(stts.correctSData, s.Data().([]float64), closeenoughf64) {
			t.Errorf("Expected %v. Got %v instead", stts.correctSData, s.Data())
		}

		if !sliceApproxf64(stts.correctUData, u.Data().([]float64), closeenoughf64) {
			t.Errorf("Expected %v. Got %v instead", stts.correctUData, u.Data())
		}

		if !sliceApproxf64(stts.correctVData, v.Data().([]float64), closeenoughf64) {
	...
	for i, stfs := range svdtestsFull {
		T = New(WithShape(stfs...), WithBacking(RandomFloat64(stfs.TotalSize())))
		T2 = T.Clone().(*Dense)
		...
		if !sliceApproxf64(s.Data().([]float64), svd.Values(nil), closeenoughf64) {
