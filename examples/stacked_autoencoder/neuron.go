...
func MakeNeuron(inputs, outputs, batchSize int, g *ExprGraph, fn initFn) Neuron {
	w := NewMatrix(g, tensor.Float64, WithShape(inputs, outputs), WithInit(fn()))

	var b *Node
	if batchSize == 1 {
		b = NewVector(g, tensor.Float64, WithShape(outputs), WithInit(Zeroes()))
	} else {
		b = NewMatrix(g, tensor.Float64, WithShape(batchSize, outputs), WithInit(Zeroes()))
	}
